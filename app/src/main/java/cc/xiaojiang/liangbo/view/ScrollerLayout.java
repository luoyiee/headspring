    ...
    protected void onLayout(boolean b, int i, int i1, int i2, int i3) {
        ...
        if (b) {
            for (int j = 0; j < childCount; j++) {
                ...
                //左上右下(横向摆放)
                if (direction == 1) {
                    childView.layout(j * childView.getMeasuredWidth(), 0, childView
                            .getMeasuredWidth() * (j + 1), childView.getMeasuredHeight());
                } else { //左上右下(纵向摆放)
                    childView.layout(0, j * childView.getMeasuredHeight(), childView
                            .getMeasuredWidth(), childView.getMeasuredHeight() * (j + 1));
                }
            }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:
                ...
                if (direction == 1) {
                    ...
                    mScrollY = (int) (mYLastMove - mYMove);
                    Log.d(TAG, "onMove:" + getScrollY() + " ,scrolledY:" + mScrollY);
                    //此判断只针对向右滑动时（即第一页就向下滑动）0 + 负数
                    if (getScrollY() + mScrollY < topBorder) { //判定为第一页时向下滑动
                    ...
                    else if (getScrollY() + getHeight() + mScrollY > bottomBorder) {
